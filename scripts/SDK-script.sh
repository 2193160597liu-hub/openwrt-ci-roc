...
release_package_name() {
  ...
  case "$safe_package_name" in
    ...
    *.apk)
      ...
      printf '%s-%s.apk\n' "${safe_package_name%.apk}" "$package_arch"
      ;;
    *)
      printf '%s\n' "$safe_package_name"
      ;;
...
copy_artifacts() {
  ...
  log "Copied $copied_count selected package files to $OUTPUT_DIR with package architecture suffixes; skipped $skipped_count dependency files"
