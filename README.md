...
## 单独编译软件包
- 点击 `Actions` 运行 `Build-Packages`，`sdk_version` 可选择 `main` 主线 snapshots，或 `23.05`、`24.10`、`25.12` 系列的最新稳定版 SDK。
...
- 两个架构共用同一个 `sdk_version` 选项；脚本会按所选版本和架构从 `https://downloads.openwrt.org/` 自动查找对应 SDK。
...
- 编译的软件包来源固定为：
  ...
  - `https://github.com/laipeng668/packages` 的 `master` 分支：`lang/golang`
...
- 编译产物会上传到本次 workflow 的 `Artifacts`，并以 `.ipk` 保留原始软件包名、`.apk` 追加软件包架构后缀、文件名已将 `~` 规范化为 `-` 的方式发布到 `Packages` 这个 [Releases](https://github.com/laipeng668/openwrt-ci-roc/releases/tag/Packages)。
